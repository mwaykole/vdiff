# vdiff Dockerfile
# vLLM-compatible serving for Diffusion LLMs
#
# Build:
...
#
# Run:
#   docker run --gpus all -p 8000:8000 vdiff --model GSAI-ML/LLaDA-8B-Instruct
#
# For CPU-only:
...
# Install vdiff and dependencies
...
RUN groupadd --gid 1000 vdiff && \
    useradd --uid 1000 --gid vdiff --shell /bin/bash --create-home vdiff && \
    chown -R vdiff:vdiff /workspace
...
RUN mkdir -p ${HF_HOME} && chown -R vdiff:vdiff /workspace/.cache
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=120s --retries=3 \
    CMD curl -f http://localhost:${VDIFF_PORT}/health/live || exit 1

# Expose port
EXPOSE 8000

# Set entrypoint
ENTRYPOINT ["python", "-m", "vdiff.entrypoints.openai.api_server"]
